...
internal class DateInputType(IHtmlInputElement input, string name) : BaseInputType(input, name, true)
{
  public override ValidationErrors Check(IValidityState current)
  {
    string str = this.Input.Value;
    DateTime? date = DateInputType.ConvertFromDate(str);
    DateTime? min = DateInputType.ConvertFromDate(this.Input.Minimum);
    DateTime? max = DateInputType.ConvertFromDate(this.Input.Maximum);
  ...
  {
    return string.Format((IFormatProvider) CultureInfo.InvariantCulture, "{0:0000}-{1:00}-{2:00}", (object) value.Year, (object) value.Month, (object) value.Day);
  }
