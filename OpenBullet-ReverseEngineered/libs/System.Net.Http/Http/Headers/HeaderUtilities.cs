...
internal static class HeaderUtilities
{
  ...
  internal static double? GetQuality(ObjectCollection<NameValueHeaderValue> parameters)
  {
    ...
    if (valueHeaderValue != null)
    {
      ...
      if (NetEventSource.Log.IsEnabled())
        NetEventSource.PrintError(NetEventSource.ComponentType.Http, string.Format((IFormatProvider) CultureInfo.InvariantCulture, SR.net_http_log_headers_invalid_quality, (object) valueHeaderValue.Value));
    }
    return new double?();
  }
  ...
  internal static bool IsValidEmailAddress(string value)
  {
    ...
    {
      if (NetEventSource.Log.IsEnabled())
        NetEventSource.PrintError(NetEventSource.ComponentType.Http, string.Format((IFormatProvider) CultureInfo.InvariantCulture, SR.net_http_log_headers_wrong_email_format, (object) value, (object) ex.Message));
    }
    return false;
  }
