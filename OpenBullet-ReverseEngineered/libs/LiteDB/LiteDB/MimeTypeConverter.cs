...
{
  private static IDictionary<string, string> _mappings = (IDictionary<string, string>) new Dictionary<string, string>((IEqualityComparer<string>) StringComparer.OrdinalIgnoreCase)
  {
    {
      ".323",
      "text/h323"
  ...
  {
    string key = !string.IsNullOrEmpty(filename) ? Path.GetExtension(filename) : throw new ArgumentNullException(nameof (filename));
    if (!key.StartsWith("."))
      key = "." + key;
    string str;
    return !MimeTypeConverter._mappings.TryGetValue(key, out str) ? "application/octet-stream" : str;
  }
}
