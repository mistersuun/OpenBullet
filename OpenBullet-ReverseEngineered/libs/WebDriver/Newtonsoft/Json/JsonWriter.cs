...
internal abstract class JsonWriter : IDisposable
{
  ...
  internal async Task InternalWriteEndAsync(
  ...
  public Task WriteTokenAsync(JsonToken token, object value, CancellationToken cancellationToken = default (CancellationToken))
  {
    ...
    switch (token)
    {
      ...
      case JsonToken.Integer:
        ValidationUtils.ArgumentNotNull(value, nameof (value));
        return !(value is BigInteger bigInteger) ? this.WriteValueAsync(Convert.ToInt64(value, (IFormatProvider) CultureInfo.InvariantCulture), cancellationToken) : this.WriteValueAsync((object) bigInteger, cancellationToken);
    ...
    CancellationToken cancellationToken)
  {
    ...
    switch (token)
    {
      ...
      case JsonToken.PropertyName:
        return value is string ? this.InternalWritePropertyNameAsync((string) value, cancellationToken) : throw new ArgumentException("A name is required when setting property name state.", nameof (value));
      case JsonToken.Comment:
    ...
    CancellationToken cancellationToken)
  {
    switch (typeCode)
    {
      ...
      default:
        TypeInformation typeInformation = value is IConvertible convertable ? ConvertUtils.GetTypeInformation(convertable) : throw JsonWriter.CreateUnsupportedTypeException(writer, value);
        ...
        return JsonWriter.WriteValueAsync(writer, typeCode1, type, cancellationToken);
    }
  }
