    ...
    do
    {
      switch (this._currentState)
      {
        ...
        case JsonReader.State.PostValue:
          ...
          if (postValueAsync.Status == TaskStatus.RanToCompletion)
            continue;
          goto label_7;
...
label_12:
    ...
    return false;
label_19:
    throw JsonReaderException.Create((JsonReader) reader, "After parsing a value an unexpected character was encountered: {0}.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) currentChar));
  }
...
label_41:
    ...
    return false;
label_6:
    configuredTaskAwaitable = jsonTextReader.ParseStringAsync(currentChar, ReadType.Read, cancellationToken).ConfigureAwait(false);
    await configuredTaskAwaitable;
    return true;
label_8:
...
label_43:
    ...
    jsonTextReader.ParseNumber(ReadType.Read);
    return true;
  }
    ...
    do
    {
      ...
      else if (ch != '"' && ch != '\'')
      {
        if (ch == '\\')
        {
          ...
          char currentChar = reader._chars[charPos];
          ++charPos;
          char writeChar;
          switch (currentChar)
          {
            ...
            default:
              reader._charPos = charPos;
              throw JsonReaderException.Create((JsonReader) reader, "Bad JSON escape sequence: {0}.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) ("\\" + currentChar.ToString())));
          }
          reader.EnsureBufferNotEmpty();
...
label_11:
    switch (currentChar)
    {
      ...
      default:
        ...
        if (currentChar != '(')
          throw JsonReaderException.Create((JsonReader) reader, "Unexpected character while parsing constructor: {0}.".FormatWith((IFormatProvider) CultureInfo.InvariantCulture, (object) currentChar));
        endPosition = reader._charPos;
        break;
    }
label_20:
  ...
  {
    JsonTextReader reader = this;
    char firstChar = reader._chars[reader._charPos];
    char quoteChar;
    ...
    {
      if (!reader.ValidIdentifierChar(firstChar))
    ...
    string propertyName = reader.NameTable == null ? reader._stringReference.ToString() : reader.NameTable.Get(reader._stringReference.Chars, reader._stringReference.StartIndex, reader._stringReference.Length) ?? reader._stringReference.ToString();
...
label_6:
        ...
        return (object) null;
label_7:
        await reader.ParseStringAsync(currentChar, readType, cancellationToken).ConfigureAwait(false);
...
label_13:
        ...
        {
          ++reader._charPos;
          throw reader.CreateUnexpectedCharacterException(currentChar);
        }
...
label_34:
        throw reader.CreateUnexpectedCharacterException(currentChar);
      case JsonReader.State.PostValue:
        if (await reader.ParsePostValueAsync(true, cancellationToken))
          return (object) null;
    ...
    switch (reader._currentState)
    {
      ...
      case JsonReader.State.Constructor:
        ...
        do
        {
          ...
          return new bool?();
label_7:
          await reader.ParseStringAsync(currentChar, ReadType.Read, cancellationToken).ConfigureAwait(false);
...
label_10:
          ...
          bool flag = !(reader.Value is BigInteger) ? Convert.ToBoolean(reader.Value, (IFormatProvider) CultureInfo.InvariantCulture) : (BigInteger) reader.Value != 0L;
...
label_5:
        ...
        return (byte[]) null;
label_6:
        await reader.ParseStringAsync(currentChar, ReadType.ReadAsBytes, cancellationToken).ConfigureAwait(false);
...
label_8:
          ...
          bool flag1 = !(this.Value is BigInteger) ? Convert.ToBoolean(this.Value, (IFormatProvider) CultureInfo.InvariantCulture) : (BigInteger) this.Value != 0L;
