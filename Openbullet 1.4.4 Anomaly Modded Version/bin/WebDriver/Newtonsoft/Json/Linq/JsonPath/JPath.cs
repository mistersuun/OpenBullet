...
internal class JPath
{
  ...
  private PathFilter ParseQuery(char indexerCloseChar, bool scan)
  {
    ...
    ++this._currentIndex;
    QueryExpression expression = this.ParseExpression(scan);
    ++this._currentIndex;
    this.EnsureLength("Path ended with open indexer.");
    this.EatWhitespace();
    if ((int) this._expression[this._currentIndex] != (int) indexerCloseChar)
  ...
  private object ParseSide(bool scan)
  {
    ...
    if (this.TryParseExpression(scan, out expressionPath))
    {
      this.EatWhitespace();
  ...
  private QueryExpression ParseExpression(bool scan)
  {
    ...
    while (this._currentIndex < this._expression.Length)
    {
      object side = this.ParseSide(scan);
