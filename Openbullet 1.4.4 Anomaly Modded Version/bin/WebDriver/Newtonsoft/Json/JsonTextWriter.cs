...
internal class JsonTextWriter : JsonWriter
{
    ...
    CancellationToken cancellationToken)
  {
    ...
    return task.Status == TaskStatus.RanToCompletion ? this._writer.WriteAsync(value, cancellationToken) : this.WriteValueInternalAsync(task, value, cancellationToken);
  }
    ...
    CancellationToken cancellationToken)
  {
    ...
    return task.Status == TaskStatus.RanToCompletion ? this.WriteDigitsAsync(uvalue, negative, cancellationToken) : this.WriteIntegerValueAsync(task, uvalue, negative, cancellationToken);
  }
  ...
  internal Task DoWritePropertyNameAsync(string name, CancellationToken cancellationToken)
  {
    ...
    return task2.Status == TaskStatus.RanToCompletion ? this._writer.WriteAsync(':', cancellationToken) : JavaScriptUtils.WriteCharAsync(task2, this._writer, ':', cancellationToken);
  }
  ...
  internal Task DoWriteStartArrayAsync(CancellationToken cancellationToken)
  {
    ...
    return task.Status == TaskStatus.RanToCompletion ? this._writer.WriteAsync('[', cancellationToken) : this.DoWriteStartArrayAsync(task, cancellationToken);
  }
  ...
  internal Task DoWriteStartObjectAsync(CancellationToken cancellationToken)
  {
    ...
    return task.Status == TaskStatus.RanToCompletion ? this._writer.WriteAsync('{', cancellationToken) : this.DoWriteStartObjectAsync(task, cancellationToken);
  }
  ...
  internal Task DoWriteUndefinedAsync(CancellationToken cancellationToken)
  {
    ...
    return task.Status == TaskStatus.RanToCompletion ? this._writer.WriteAsync(JsonConvert.Undefined, cancellationToken) : this.DoWriteUndefinedAsync(task, cancellationToken);
  }
  ...
  {
    Task task = this.InternalWriteValueAsync(JsonToken.String, cancellationToken);
    return task.Status == TaskStatus.RanToCompletion ? this.WriteEscapedStringAsync(value.OriginalString, true, cancellationToken) : this.WriteValueNotNullAsync(task, value, cancellationToken);
  }
  ...
  internal Task DoWriteRawValueAsync(string json, CancellationToken cancellationToken)
  {
    ...
    return task.Status == TaskStatus.RanToCompletion ? this.WriteRawAsync(json, cancellationToken) : this.DoWriteRawValueAsync(task, json, cancellationToken);
  }
  ...
  private void WriteIntegerValue(ulong uvalue, bool negative)
  {
    if (!negative & uvalue <= 9UL)
      this._writer.Write((char) (48UL /*0x30*/ + uvalue));
    else
  ...
  private int WriteNumberToBuffer(ulong value, bool negative)
  {
    ...
    do
    {
      this._writeBuffer[--num] = (char) (48UL /*0x30*/ + value % 10UL);
      value /= 10UL;
    }
    while (value != 0UL);
    return buffer;
