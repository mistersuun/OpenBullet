...
// Type: RuriLib.CaptchaServices.RuCaptcha
...
public class RuCaptcha(string apiKey, int timeout) : CaptchaService(apiKey, timeout)
{
  public override double GetBalance()
  {
    using (CWebClient cwebClient = new CWebClient())
    {
      ...
      return double.Parse(cwebClient.DownloadString($"http://rucaptcha.com/res.php?key={this.ApiKey}&action=getbalance"), (IFormatProvider) CultureInfo.InvariantCulture);
    }
  }
  ...
  public override string SolveRecaptcha(string siteKey, string siteUrl)
  {
    using (CWebClient cwebClient = new CWebClient())
    {
      if (this.Timeout > 0)
        cwebClient.Timeout = this.Timeout;
      string message1 = cwebClient.DownloadString($"http://rucaptcha.com/in.php?key={this.ApiKey}&method=userrecaptcha&googlekey={siteKey}&pageurl={siteUrl}");
      ...
      {
        Thread.Sleep(5000);
        string message2 = cwebClient.DownloadString($"http://rucaptcha.com/res.php?key={this.ApiKey}&action=get&id={this.TaskId}");
  ...
  public override string SolveCaptcha(Bitmap bitmap)
  {
    using (CWebClient cwebClient = new CWebClient())
    {
      ...
      string message1 = cwebClient.UploadString("http://rucaptcha.com/in.php", $"key={this.ApiKey}&method=base64&regsense=1&body={this.EscapeLongString(this.GetBase64(bitmap, ImageFormat.Bmp))}");
      this.TaskId = message1.StartsWith("OK") ? (object) message1.Split('|')[1] : throw new Exception(message1);
      this.Status = CaptchaService.CaptchaStatus.Processing;
      DateTime now = DateTime.Now;
      while (this.Status == CaptchaService.CaptchaStatus.Processing && (DateTime.Now - now).TotalSeconds < (double) this.Timeout)
