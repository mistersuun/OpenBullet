// Decompiled with JetBrains decompiler
// Type: AngleSharp.Html.InputTypes.CheckedInputType
// Assembly: AngleSharp, Version=0.13.0.0, Culture=neutral, PublicKeyToken=e83494dcdc6d31ea
// MVID: 339EDA8C-04A7-4BF4-9B64-DA1BC68B2CFB
// Assembly location: C:\Users\futiliter\Documents\Projects\OpenBullet\OpenBullet-ReverseEngineered\libs\AngleSharp.dll

using AngleSharp.Common;
using AngleSharp.Html.Dom;

#nullable disable
namespace AngleSharp.Html.InputTypes;

internal class CheckedInputType(IHtmlInputElement input, string name) : BaseInputType(input, name, true)
{
  public override ValidationErrors Check(IValidityState current)
  {
    ValidationErrors validationErrors = BaseInputType.GetErrorsFrom(current) & ~ValidationErrors.ValueMissing;
    if (this.Input.IsRequired && !this.Input.IsChecked)
      validationErrors ^= ValidationErrors.ValueMissing;
    return validationErrors;
  }

  public override void ConstructDataSet(FormDataSet dataSet)
  {
    if (!this.Input.IsChecked)
      return;
    string str = this.Input.HasValue ? this.Input.Value : Keywords.On;
    dataSet.Append(this.Input.Name, str, this.Input.Type);
  }
}
