...
internal static class EnumUtils
{
  ...
  private static EnumInfo InitializeValuesAndNames(StructMultiKey<Type, NamingStrategy> key)
  {
    ...
    for (int count = 0; count < names.Length; ++count)
    {
      ...
      string name2 = field.GetCustomAttributes(typeof (EnumMemberAttribute), true).Cast<EnumMemberAttribute>().Select<EnumMemberAttribute, string>((Func<EnumMemberAttribute, string>) (a => a.Value)).SingleOrDefault<string>() ?? field.Name;
      ...
      strArray[count] = key.Value2 != null ? key.Value2.GetPropertyName(name2, false) : name2;
    }
