...
internal class QueryGreater : Query
{
  ...
  internal override IEnumerable<IndexNode> ExecuteIndex(IndexService indexer, CollectionIndex index)
  {
    ...
    if (node != null)
    {
      for (; node != null; node = indexer.GetNode(node.Next[0]))
      {
        if (node.Key.Type == this._value.Type || node.Key.IsNumber && this._value.IsNumber)
        {
          int num = node.Key.CompareTo(this._value);
