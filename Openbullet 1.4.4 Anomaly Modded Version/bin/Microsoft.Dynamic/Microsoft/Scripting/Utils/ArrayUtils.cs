...
public static class ArrayUtils
{
  ...
  {
    ContractUtils.RequiresNotNull((object) input, nameof (input));
    ContractUtils.RequiresNotNull((object) conv, nameof (conv));
    TOutput[] outputArray = new TOutput[input.Length];
    for (int index = 0; index < input.Length; ++index)
      outputArray[index] = conv(input[index]);
    return outputArray;
  }
  ...
  {
    return list is T[] objArray ? objArray : ArrayUtils.MakeArray<T>(list);
  }

  public static TResult[] ToArray<TElement, TResult>(
    ICollection<TElement> list,
    Func<TElement, TResult> convertor)
  {
    if (!(list is TResult[] array))
    {
      array = new TResult[list.Count];
      int num = 0;
      foreach (TElement element in (IEnumerable<TElement>) list)
        array[num++] = convertor(element);
    }
    return array;
  }
  ...
  {
    Array.Resize<T>(ref array, array != null ? array.Length + 1 : 1);
  ...
  public static T[] AppendRange<T>(T[] array, IList<T> items, int additionalItemCount)
  {
    ...
    Array.Resize<T>(ref array, length + items.Count + additionalItemCount);
    int index = 0;
  ...
  {
    return array.GetValueHashCode<T>(0, array.Length);
  }
  ...
  public static int GetValueHashCode<T>(this T[] array, int start, int count)
  {
    ...
    if (count == 0)
      return 0;
    int valueHashCode = array[start].GetHashCode();
    for (int index = 1; index < count; ++index)
      valueHashCode = (valueHashCode << 5 | valueHashCode >> 27) ^ array[start + index].GetHashCode();
